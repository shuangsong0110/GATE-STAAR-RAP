...
if(T){
  ...
  STAAR_B_Binary_SPA_survival <- function(G,mac,
    ...
    if(nsnp00-length(ind_ld)==1){
      ...
      if(T){
        ...
        para2 <- t(compute_k_theta_v2(mu0, d1=pmax(-log10(1-exp(-mu0))+0.1,3), d2=5))
        #para3 <- compute_k_theta_v(mu0, d1=20, d2=40)
      ...
      for (i in 1:wn) {
        ...
        if(T){
          ...
          tmp00 <- pv_skat_ld5(#mu=c(obj_nullmodel$mu), 
          ...
          if(inherits(aa, "try-error")){
            ...
            tmp <- pv_skat_ld4(#mu=c(obj_nullmodel$mu), 
              ...
              #y=c(obj_nullmodel$y), g=G, 
              #R_pois=abs(R00),
              kk=kk, theta=theta, mu0=mu0, varRatio=varRatio)$pv
          }
          # print('pvpvpv')
          # print(c(tmp, tmp0, tmp.unadj))
          res[i] <- max(c(tmp, tmp0, tmp.unadj),na.rm=T)
          if(T){
            tmp00 <- pv_skat_ld5(#mu=c(obj_nullmodel$mu), 
              weights=weights_S[,i]^2/sum(weights_S[,i]^2), 
              score_skat0_unweight=score_skat0_unweight,cs=cs,
              #y=c(obj_nullmodel$y), g=G, 
              #R_pois=abs(R00),
              kk=kk2, theta=theta2, mu0=mu0, varRatio=varRatio, useLD=F, R_pois=abs(R00))
            tmp0 <- tmp00$pv
            #tmp.unadj <- tmp00$pv.unadj
            aa <- try( tmp <- pv_skat_ld5(#mu=c(obj_nullmodel$mu), 
              weights=weights_S[,i]^2/sum(weights_S[,i]^2), 
            ...
            if(inherits(aa, "try-error")){
              ...
              tmp <- pv_skat_ld4(#mu=c(obj_nullmodel$mu), 
                ...
                #y=c(obj_nullmodel$y), g=G, 
                #R_pois=abs(R00),
                kk=kk2, theta=theta2, mu0=mu0, varRatio=varRatio)$pv
            }
          }
          res[i] <- max(c(tmp, tmp0, res[i]),na.rm=T)
          if(F){
            tmp00 <- pv_skat_ld5(#mu=c(obj_nullmodel$mu), 
              weights=weights_S[,i]^2/sum(weights_S[,i]^2), 
              score_skat0_unweight=score_skat0_unweight,cs=cs,
              #y=c(obj_nullmodel$y), g=G, 
              #R_pois=abs(R00),
              kk=kk3, theta=theta3, mu0=mu0, varRatio=varRatio, useLD=F, R_pois=abs(R00))
            tmp0 <- tmp00$pv
            #tmp.unadj <- tmp00$pv.unadj
            aa <- try( tmp <- pv_skat_ld5(#mu=c(obj_nullmodel$mu), 
              weights=weights_S[,i]^2/sum(weights_S[,i]^2), 
            ...
            if(inherits(aa, "try-error")){
              ...
              tmp <- pv_skat_ld4(#mu=c(obj_nullmodel$mu), 
                ...
                #y=c(obj_nullmodel$y), g=G, 
                #R_pois=abs(R00),
                kk=kk3, theta=theta3, mu0=mu0, varRatio=varRatio)$pv
            }
          }
          res[i] <- max(c(tmp, tmp0, res[i]),na.rm=T)
  ...
  if(T){
    ...
    pv_skat_ld4 <- function(weights,score_skat0_unweight,  cs, kk, theta, mu0, varRatio=1){
      ...
      #w1 <-  weights*theta
      #library(mgcv)
      pv.unadj <- psum.chisq(sum(score_skat0), df=rep(1,length(theta)),lb=weights*mu0)
      ...
      return(list(pv=pv_wgamma(alpha=kk, beta=1, weights=weights*theta, cut=sum(score_skat0), R_pois=NA, pv.unadj=pv.unadj)$pv,
                  pv.unadj=pv.unadj))
    }
    ...
    pv_skat_ld5 <- function(weights,score_skat0_unweight,  cs, kk, theta, mu0, varRatio=1, useLD=T, R_pois){
      ...
      # pv_wgamma(alpha=abs(shape_params_w), beta=1/abs(scale_params_w_new), 
      #           w=sign(shape_params_w*scale_params_w_new ), cut=sum(score_skat0))$pv
      return(list(pv=pv_wgamma(alpha=kk_new, beta=1, weights=weights_new*theta_new, cut=sum(score_skat0), R_pois=NA, pv.unadj=pv.unadj)$pv,
                  pv.unadj=pv.unadj))
    }
  ...
  if(T){
    compute_k_theta <- function(mu, d1=5, d2=7, iter=10){
    ...
    ff1 <- function(mu,cut){
      return( ppois(sqrt(cut)+mu, lambda = mu,lower.tail=F)+ppois(-sqrt(cut)-mu, lambda=mu)
      )
    }
    compute_k_theta_v2 <- function(mu, d1=5, d2=7, iter=10){
      # 确保 mu 和 d1 长度相同
      n <- length(mu)
      if(length(d1) != n) {
        d1 <- rep_len(d1, n)
      ...
      # 对每个 mu 和 d1 组合应用计算
      result <- mapply(function(mu_i, d1_i) {
        d2_i <- d2
        
        cut1 <- floor(sqrt(uniroot(ff1_pv, 
                                   interval=c(0, max(5000, 500*mu_i)), 
                                   mu=mu_i, pv=10^(-d1_i))$root))^2
        cut2 <- floor(sqrt(uniroot(ff1_pv, 
                                   interval=c(0, max(5000, 500*mu_i)), 
                                   mu=mu_i, pv=10^(-d2_i))$root))^2
        ...
        p1 <- ff1(mu_i, cut1)
        p2 <- ff1(mu_i, cut2)
        kk <- 4*mu_i*(2*mu_i+1)^3/(8*mu_i^2+22*mu_i+1)^2
        theta <- mu_i/kk
        
        optim(c(kk, theta), 
              cut1=cut1, cut2=cut2, p1=p1, p2=p2, 
              fn=ff_gamma_optim)$par
      }, 
      mu, d1, 
      SIMPLIFY=FALSE)
    ...
    # mu_vec <- c(0.5, 1, 1.5)
    # d1_vec <- c(5, 5, 6)
    # result <- compute_k_theta(mu=mu_vec, d1=d1_vec, d2=7)
    
    ff1_pv <- function(mu,cut, pv){
      return( ppois(sqrt(cut)+mu, lambda = mu,lower.tail=F)+ppois(-sqrt(cut)-mu, lambda=mu)-pv
      )
    }
    ...
    ff_gamma_optim <- function(xx, cut1, cut2, p1, p2){
      kk <- xx[1]
      theta <- xx[2]
      return((-log10(pgamma(cut1, shape=kk, scale=theta,lower.tail=F))+log10(p1))^2+
               (-log10(pgamma(cut2, shape=kk, scale=theta,lower.tail=F))+log10(p2))^2)
    }
